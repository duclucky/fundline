# AGENTS.md - Fundline (fundline.xyz)
...
Detailed, per-topic guidance lives in `.Codex/rules/`. Files there with no `paths:`
frontmatter load automatically at session start. Read them before working:

- `.Codex/rules/project-overview.md` - what Fundline is and current build status
- `.Codex/rules/architecture.md` - tech stack, directory map, agent + x402 surface
- `.Codex/rules/onchain-reference.md` - contract addresses, chainId, RPC, CCTP, decimals
- `.Codex/rules/commands.md` - install, run, test, contract deploy
- `.Codex/rules/coding-conventions.md` - hard requirements (style, language, invariants)
- `.Codex/rules/glossary.md` - domain terms (PaymentRouter, x402, CCTP, SBT, etc.)
- `.Codex/rules/gotchas.md` - fragile areas and "do not touch"
- `.Codex/rules/git-workflow.md` - git layout, commit style, CI/CD
- `.Codex/rules/escrow-spec.md` - FundlineEscrow spec (state machine, functions, invariant, integration)

To make a rule load only for specific files instead of every session, add a `paths:`
glob frontmatter block to that rule file (Codex rule spec). None are scoped today
because all current rules are project-wide.

## Subagents

Project subagents live in `.Codex/agents/`. Delegate to them to keep this session's
...
Packaged procedures in `.Codex/skills/<name>/SKILL.md`, invoked with `/<name>`. They encode
...
- `create-workflow` - end-to-end procedure to design and ship a new AI workflow (adapt a
  community chain -> CheapKeyAI executor -> /run wiring -> frontend -> tests -> deploy). Read it
  before building any new workflow. Provider details: `cheapkeyai-api`, including the provider
  contract, model-id map, and price table for workflow model calls.
...
Keep `.Codex/memory.md` current. Update it as decisions land, and when the session is
winding down or the context is about to be summarized, flush any new decisions, TODOs, and
gotchas into it (dated, English, no duplication of AGENTS.md or the rule files). A
`PreCompact` hook in `.Codex/settings.json` prints a reminder before compaction, but it is
