...
function handleLlmsTxt(req, res) {
  ...
  const lines = [
    ...
    "",
    "## Fastest path: turnkey clients (do NOT hand-roll the payment)",
    "Ready-to-run scripts do discover -> pay -> run for you. Do not write your own on-chain",
    "code. Your key never leaves your machine; use a dedicated agent wallet with a small",
    "balance.",
    "",
    "RECOMMENDED (gas-free, reliable, best for repeated runs) - Circle Gateway:",
    "  curl -O " + base + "/fundline-agent-gateway.js",
    "  npm i @circle-fin/x402-batching",
    "  FUNDLINE_PRIVATE_KEY=0xYourAgentKey GATEWAY_DEPOSIT=1 node fundline-agent-gateway.js  # deposit once",
    "  FUNDLINE_PRIVATE_KEY=0xYourAgentKey node fundline-agent-gateway.js <slug> normal \"<prompt>\"",
    "Deposit once on-chain, then every run is paid off-chain (no gas, no per-run RPC).",
    "",
    "SIMPLE (no Circle SDK, one USDC transfer per run) - x402:",
    "  curl -O " + base + "/fundline-agent.js",
    "  npm i ethers",
    "  FUNDLINE_PRIVATE_KEY=0xYourAgentKey node fundline-agent.js <slug> normal \"<prompt>\"",
    "Note: this sends an on-chain tx per run. The public Arc testnet RPC is heavily",
    "rate-limited; for more than occasional runs set ARC_RPC_URL to a less-throttled endpoint,",
    "or use the Gateway client above.",
    "",
