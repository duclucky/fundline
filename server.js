...
function handleLlmsTxt(req, res) {
  ...
  const lines = [
    "# Fundline",
    "",
    "## Run a workflow now (one command)",
    "curl -O " + base + "/fundline-agent-gateway.js && npm i @circle-fin/x402-batching",
    "FUNDLINE_PRIVATE_KEY=<your agent wallet key> node fundline-agent-gateway.js <slug> normal \"<your prompt>\"",
    "Get <slug> from GET " + base + "/api/workflows. This pays per run in USDC from your own",
    "wallet (gas-free) and prints the output. No sign-in, no account, no API key. Everything",
    "below is optional detail.",
    "",
    ...
    "",
    "## Turnkey clients (details)",
    "Recommended: fundline-agent-gateway.js (the one-command block above) - Circle Gateway,",
    "gas-free. First time only, deposit once: FUNDLINE_PRIVATE_KEY=0x... GATEWAY_DEPOSIT=1",
    "node fundline-agent-gateway.js. After that, runs need no deposit and no gas.",
    "Alternative without the Circle SDK: fundline-agent.js (x402, needs ethers). It sends one",
    "on-chain tx per run, and the public Arc testnet RPC is heavily rate-limited, so set",
    "ARC_RPC_URL to a less-throttled endpoint or prefer the Gateway client.",
    "Your key never leaves your machine; use a dedicated agent wallet with a small balance.",
    "",
    "## The whole flow in 3 calls (if you build it yourself)",
