...
- Use `cheap-5.6-sol`, `cheap-5.6-terra`, and `cheap-5.6-sol` as the normal, plus, and pro final-node defaults. The production CheapKeyAI key group had no route for `gpt-5.6-luna`, which caused the 2026-08-25 Client Research run to fail after its earlier `gpt-4o-mini` step succeeded. Model IDs and prices are centralized in `cheapkey-models.js`.
...
- Refunded and failed browser workflow jobs restore the primary action as `Try again`; successful jobs restore it as `Run again`. Terminal states never auto-rerun or auto-charge.
