...
assert(server.includes('normal: String(process.env.WORKFLOW_FINAL_MODEL_NORMAL || "cheap-5.6-sol").trim()'));
assert(server.includes('plus: String(process.env.WORKFLOW_FINAL_MODEL_PLUS || "cheap-5.6-terra").trim()'));
assert(server.includes('pro: String(process.env.WORKFLOW_FINAL_MODEL_PRO || "cheap-5.6-sol").trim()'));
assert.equal(server.includes('|| "gpt-5.6-luna"'), false);
assert.equal(server.includes('|| "gpt-5.6-terra"'), false);
assert.equal(server.includes('|| "gpt-5.6-sol"'), false);
...
assert(env.includes("WORKFLOW_FINAL_MODEL_NORMAL=cheap-5.6-sol"));
assert(env.includes("WORKFLOW_FINAL_MODEL_PLUS=cheap-5.6-terra"));
assert(env.includes("WORKFLOW_FINAL_MODEL_PRO=cheap-5.6-sol"));
