...
const CONTRACT_PATH = path.join(ROOT, "contracts", "PaymentRouter.sol");
...
async function main() {
  ...
  const { abi, bytecode } = compilePaymentRouter();
  ...
  console.log(`Deploying PaymentRouter on chain ${network.chainId}`);
...
function compilePaymentRouter() {
  const source = fs.readFileSync(CONTRACT_PATH, "utf8");
  const input = {
    language: "Solidity",
    sources: {
      "PaymentRouter.sol": { content: source },
    },
    settings: {
  ...
  if (!contract?.abi || !contract?.evm?.bytecode?.object) {
    throw new Error("PaymentRouter compile output is missing ABI or bytecode.");
  }
